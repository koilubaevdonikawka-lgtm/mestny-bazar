...
function Home() {
  ...
  return (
    // Задача: убрать лишнее пустое пространство снизу — min-h-screen раньше
    // растягивало обёртку до полной высоты экрана независимо от реального
    // контента (подтверждено измерением: на viewport 390×1200 контент
    // заканчивался на 978px, обёртка всё равно занимала 1200px — 222px
    // чистой пустоты). Секции ниже уже не содержат flex-1 (каталог товаров,
    // который раньше растягивался на всю высоту, удалён в одной из
    ...
    <div className="flex flex-col">
      ...
      {/* Горизонтальная панель основных категорий, под шапкой. Клик по
          ...
          уходит со страницы) и обновляет центральную сетку подкатегорий
          ниже; клик по категории без подкатегорий ведёт сразу на её
          страницу товаров (см. subcategoryCountByParentId выше — иначе
          товары таких категорий стали бы недостижимы через эту панель).
          Sticky прямо под шапкой (Этап: владелец продукта явно попросил
          закрепить панель при скролле — было намеренно НЕ sticky, решение
          изменено). top — реальная высота шапки: h-16 (4rem) + её нижний
          border-b (1px) + её собственный pt-safe
          (env(safe-area-inset-top), см. SiteHeader.tsx/styles.css) — без
          учёта border/safe-area эта панель наезжала бы на шапку ровно на
          эту величину (на native/Capacitor с notch — на весь отступ
          safe-area). z-30, ниже шапки (z-40), чтобы шапка оставалась
          поверх при одновременном наложении. */}
      ...
      {activeCategory && activeSubcategories.length > 0 && (
        // Этап: убрать лишний отступ между sticky-панелью категорий и
        // первым рядом карточек (было pt-8=32px + SubcategoryGrid's own
        // p-1=4px = 36px, измерено на проде) — только верхний отступ на
        // мобильном брейкпоинте (pt-3=12px, итог 16px), нижний (pb-8,
        // держит отступ в конце страницы — не размечен, не трогаю) и
        // sm:-варианты не тронуты.
        <section className="mx-auto max-w-7xl px-3 pt-3 pb-8 w-full sm:px-6 sm:pt-12 sm:pb-12">
