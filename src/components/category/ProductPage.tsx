...
import { z } from "zod";
import { ArrowLeft, Loader2, Search, ShoppingBasket, SlidersHorizontal } from "lucide-react";
...
/**
 * Общая логика "категория/подкатегория → товары" (фильтры, сортировка,
 * поиск, сетка, брэдкрамбы) — используется двумя разными маршрутами
 * (`/category/$slug` и `/category/$categorySlug/subcategory/$subcategorySlug`),
 * которые для БД идентичны (categories.slug глобально уникален, категория и
 * подкатегория — одна сущность). Вынесено сюда, чтобы не дублировать эту
 * логику во втором маршруте (Этап: трёхуровневая навигация).
 */
...
export interface ProductPageProps {
  ...
  /** Route.useNavigate()'s `search` updater, thin-wrapped by each route
   * component — держит эту функцию route-agnostic (без завязки на типы
   * конкретного маршрута). */
  setSearch: (updater: (prev: CategorySearch) => CategorySearch) => void;
  /** Куда ведёт кнопка «Назад» — на главную, где теперь живёт сетка
   * подкатегорий (Этап: трёхуровневая навигация, п.3). */
  backHref: string;
}

export function ProductPage({ slug, search, setSearch, backHref }: ProductPageProps) {
  ...
  const resetFilters = () => {
    setMinPriceInput("");
    setMaxPriceInput("");
    setSearch((prev) => ({ sortBy: prev.sortBy }));
  };
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 mx-auto max-w-7xl px-4 pt-4 pb-12 w-full sm:px-6 lg:pt-12">
        {/* Кнопка «Назад» — на главную, где теперь центральная сетка
            подкатегорий (Этап: трёхуровневая навигация, требование №3). */}
        <Button asChild variant="ghost" className="mb-2 -ml-2 rounded-full">
          <Link to={backHref}>
