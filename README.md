...
Приложение поднимается строго на порту `8080` (`strictPort: true` в `vite.config.ts`) — если порт занят, `npm run dev` завершится с ошибкой вместо того, чтобы молча переключиться на другой порт. Это сделано намеренно: Google OAuth redirect (`Supabase Auth → Redirect URLs`) настроен на конкретный порт, и плавающий порт ломает вход через Google.
