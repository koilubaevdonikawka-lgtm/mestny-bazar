...
- **Тесты**: Vitest
- **Деплой**: Cloudflare Workers через Nitro (`@lovable.dev/vite-tanstack-config`); проект использует инструментарий Lovable Cloud (см. авто-генерируемые файлы в `src/integrations/supabase/`)
...
Проект содержит признаки управления через **Lovable Cloud** (файлы с пометкой `// This file is automatically generated`, `@lovable.dev/*`-зависимости) — фактический процесс деплоя и управление секретами могут быть настроены через эту платформу, а не напрямую через Wrangler.
