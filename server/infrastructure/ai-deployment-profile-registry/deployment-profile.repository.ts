...
/** In-memory deployment profile store. */
export class DeploymentProfileRepository implements IDeploymentProfileRepository {
  ...
  async save(deploymentProfile: DeploymentProfile): Promise<void> {
  ...
  async findAll(): Promise<readonly DeploymentProfile[]> {
    return Object.freeze([...this.deploymentProfiles.values()]);
  }

  async delete(deploymentProfileId: string): Promise<boolean> {
    const deploymentProfile = await this.findById(deploymentProfileId);
