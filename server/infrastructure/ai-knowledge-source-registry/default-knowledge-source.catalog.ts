...
/** Default in-memory knowledge source catalog index. */
export class DefaultKnowledgeSourceCatalog implements IKnowledgeSourceCatalog {
  ...
  async register(knowledgeSource: KnowledgeSource): Promise<void> {
  ...
  async remove(knowledgeSourceId: string): Promise<void> {
    const knowledgeSource = this.knowledgeSources.get(knowledgeSourceId.trim());
    if (!knowledgeSource) {
      return;
    }
