...
/** In-memory infrastructure profile store. */
export class InfrastructureProfileRepository implements IInfrastructureProfileRepository {
  ...
  async save(infrastructureProfile: InfrastructureProfile): Promise<void> {
  ...
  async findAll(): Promise<readonly InfrastructureProfile[]> {
    return Object.freeze([...this.infrastructureProfiles.values()]);
  }

  async delete(infrastructureProfileId: string): Promise<boolean> {
    const infrastructureProfile = await this.findById(infrastructureProfileId);
