...
/** Default in-memory resource catalog index. */
export class DefaultResourceCatalog implements IResourceCatalog {
  ...
  async remove(resourceId: string): Promise<void> {
    const resource = this.resources.get(resourceId.trim());
    if (!resource) {
      return;
    }
  ...
  async listAll(): Promise<readonly Resource[]> {
