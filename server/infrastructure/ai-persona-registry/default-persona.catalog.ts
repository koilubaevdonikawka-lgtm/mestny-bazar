...
/** Default in-memory persona catalog index. */
export class DefaultPersonaCatalog implements IPersonaCatalog {
  ...
  async register(persona: Persona): Promise<void> {
  ...
  async remove(personaId: string): Promise<void> {
    const persona = this.personas.get(personaId.trim());
    if (!persona) {
      return;
    }
  ...
  async listAll(): Promise<readonly Persona[]> {
