...
/** In-memory reliability profile store. */
export class ReliabilityProfileRepository implements IReliabilityProfileRepository {
  ...
  async save(reliabilityProfile: ReliabilityProfile): Promise<void> {
  ...
  async findAll(): Promise<readonly ReliabilityProfile[]> {
    return Object.freeze([...this.reliabilityProfiles.values()]);
  }

  async delete(reliabilityProfileId: string): Promise<boolean> {
    const reliabilityProfile = await this.findById(reliabilityProfileId);
