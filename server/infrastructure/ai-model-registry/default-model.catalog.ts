...
/** Default in-memory model catalog index. */
export class DefaultModelCatalog implements IModelCatalog {
  ...
  async remove(modelId: string): Promise<void> {
    const model = this.models.get(modelId.trim());
    if (!model) {
      return;
    }
  ...
  async listAll(): Promise<readonly Model[]> {
