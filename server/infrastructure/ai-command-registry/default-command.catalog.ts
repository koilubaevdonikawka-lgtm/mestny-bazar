...
/** Default in-memory command catalog index. */
export class DefaultCommandCatalog implements ICommandCatalog {
  ...
  async remove(commandId: string): Promise<void> {
    const command = this.commands.get(commandId.trim());
    if (!command) {
      return;
    }
  ...
  async listAll(): Promise<readonly Command[]> {
