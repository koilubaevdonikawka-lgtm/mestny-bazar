...
/** Default in-memory network profile catalog index. */
export class DefaultNetworkProfileCatalog implements INetworkProfileCatalog {
  ...
  async register(networkProfile: NetworkProfile): Promise<void> {
  ...
  async remove(networkProfileId: string): Promise<void> {
    const networkProfile = this.networkProfiles.get(networkProfileId.trim());
    if (!networkProfile) {
      return;
    }
  ...
  async listAll(): Promise<readonly NetworkProfile[]> {
