...
/** Default in-memory environment profile catalog index. */
export class DefaultEnvironmentProfileCatalog implements IEnvironmentProfileCatalog {
  ...
  async register(environmentProfile: EnvironmentProfile): Promise<void> {
  ...
  async remove(environmentProfileId: string): Promise<void> {
    const environmentProfile = this.environmentProfiles.get(environmentProfileId.trim());
  ...
  async listAll(): Promise<readonly EnvironmentProfile[]> {
