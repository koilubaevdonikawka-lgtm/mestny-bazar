...
| **Дата последнего обновления** | 2026-08-02 (Этап 5 закрыт; roadmap завершён — Этап 6 не определён, см. раздел ниже; добавлен новый модуль «Delivery Management & Pricing» со своим отдельным roadmap, см. раздел «Новый модуль вне этого roadmap» ниже) |
...
## Новый модуль вне этого roadmap: Delivery Management & Pricing

«Roadmap завершён на Этапе 5» выше относится строго к 19 модулям, перечисленным в `ADMIN_PLATFORM_MASTER_SPEC.md` §5 на момент закрытия. **Delivery Management & Pricing** — новый модуль, добавленный в `ADMIN_PLATFORM_MASTER_SPEC.md` §5 отдельно (2026-08-02, Промпт №020), с **собственным**, отдельным roadmap: [`docs/delivery/IMPLEMENTATION_ORDER.md`](../delivery/IMPLEMENTATION_ORDER.md) (3 этапа: Архитектура → Реализация ядра → Продвинутое ценообразование; Этап 1 закрыт).

Это не переоткрытие пятиэтапного roadmap выше — тот факт, что Административная платформа зафиксирована завершённой на Этапе 5, не означает запрет на появление новых модулей платформы в принципе; он означает, что расширение платформы новыми модулями происходит через отдельные, самостоятельно поставленные и самостоятельно закрываемые roadmap-документы (по аналогии с тем, как `ARCHITECTURE_BASELINE_V1.md`, §7, разрешает любой новой подсистеме расширять Baseline, не изменяя его), а не через дописывание несуществующего «Этапа 6» в этот файл — именно это различие было явно установлено при отказе от «Этапа 6» (см. раздел выше).
