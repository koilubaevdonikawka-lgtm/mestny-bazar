...
|---|---|
| **Версия** | 0.1 |
| **Статус** | Черновая архитектура — не реализовано (Этап 1 из 3: архитектура и фундамент) |
...
## Что зафиксировано в этой ревизии (Этап 1 из 3)

**Только архитектура.** Ни одна строка исполняемого кода, ни одна миграция, ни один UI-компонент, ни один API-эндпоинт не созданы этим этапом. Зафиксированы: доменная модель, Rule Engine, модель событий, контур API (как план, не как код), карта зависимостей, трёхэтапный план реализации.

Реализация (Этап 2) и продвинутое ценообразование (Этап 3) начинаются только после отдельного, явно поставленного промпта — см. [`IMPLEMENTATION_ORDER.md`](./IMPLEMENTATION_ORDER.md).
