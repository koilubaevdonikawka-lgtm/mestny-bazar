...
| **Маркетинг (`marketing.md`)** | Модель ролей `Admin-Marketing` (`AdminMarketingScopeRule`) — переиспользуется для управления тарифами (см. «Права доступа» ниже) | — | Маркетинг (если `marketing.md` введёт собственные акции, затрагивающие доставку — например, «бесплатная доставка по промокоду» — потребуется согласовать с `DeliveryTariffPolicyService`, не дублировать логику в `DiscountPolicyService`) |
| **Права доступа (`permissions.md`)** | Матрица ролей (Admin/Admin-Marketing) — Delivery не вводит новых ролей, но **расширяет** `AdminMarketingScopeRule`'s allowlist модулем `"delivery"` (Этап 3) — единственное изменение существующего кода вне `docs/delivery/`, сделанное этим модулем; см. `docs/delivery/DELIVERY_MASTER_SPEC.md` §5 | — | Права доступа (`ADMIN_PLATFORM_MASTER_SPEC.md` §8 обновлена — строка «Доставка» добавлена в сводную матрицу) |
